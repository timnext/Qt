...
main(int argc, char** argv)
{
    ...
    std::vector<PointCloud<PointXYZ>::Ptr> cloud_models;
    pcl::io::loadPCDFile("bun000.pcd", *cloud_scene);
    pcl::io::loadPCDFile("bun045.pcd",*cloud_model);
    
    std::vector<int>nan_indicies;
    pcl::removeNaNFromPointCloud(*cloud_scene, *cloud_scene, nan_indicies);
    pcl::removeNaNFromPointCloud(*cloud_model, *cloud_model, nan_indicies);
    ...
    for (const auto& cloud_model : cloud_models) {
        ...
        PPFHashMapSearch::Ptr hashmap_search(
            new PPFHashMapSearch(25.5f / 180.0f * float(M_PI), 0.05f));
    ...
    std::cout<<Eigen::Vector3f::UnitX();

    for (std::size_t model_i = 0; model_i < cloud_models.size(); ++model_i) {

        PPFRegi
            stration<PointNormal, PointNormal> ppf_registration;
        // set parameters for the PPF registration procedure
        ppf_registration.setSceneReferencePointSamplingRate(65);
